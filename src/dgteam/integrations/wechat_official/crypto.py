...
class WechatOfficialCryptoError(Exception):
    pass
...
def _pkcs7_unpad(data: bytes) -> bytes:
    if not data:
        raise WechatOfficialCryptoError("Cannot unpad an empty payload.")
    pad_len = data[-1]
    if pad_len < 1 or pad_len > BLOCK_SIZE:
        raise WechatOfficialCryptoError("Invalid PKCS7 padding.")
    if data[-pad_len:] != bytes([pad_len]) * pad_len:
        raise WechatOfficialCryptoError("Corrupted PKCS7 padding.")
    return data[:-pad_len]


class WechatOfficialCrypto:
    ...
    def _decode_aes_key(encoding_aes_key: str) -> bytes:
        ...
        if not raw:
            raise WechatOfficialCryptoError("Missing EncodingAESKey.")
        padding = "=" * ((4 - len(raw) % 4) % 4)
        try:
            decoded = base64.b64decode(raw + padding)
        except Exception as exc:  # pragma: no cover
            raise WechatOfficialCryptoError("EncodingAESKey is not valid base64.") from exc
        if len(decoded) != 32:
            raise WechatOfficialCryptoError("EncodingAESKey must decode to 32 bytes.")
        return decoded
    ...
    def verify_url_signature(self, *, signature: str, timestamp: str, nonce: str) -> bool:
        parts = [str(self.token or ""), str(timestamp or ""), str(nonce or "")]
        parts.sort()
    ...
    def decrypt_message(self, *, msg_signature: str, timestamp: str, nonce: str, encrypted: str) -> str:
        ...
        try:
            ...
            raise WechatOfficialCryptoError("Encrypted callback payload is not valid base64.") from exc

        cipher = AES.new(self.aes_key, AES.MODE_CBC, self.iv)
        decrypted = _pkcs7_unpad(cipher.decrypt(encrypted_bytes))
        if len(decrypted) < 20:
            raise WechatOfficialCryptoError("Decrypted callback payload is too short.")
        ...
        if self.app_id and receive_id != self.app_id:
            raise WechatOfficialCryptoError("Callback AppID does not match the configured AppID.")
    ...
    def encrypt_message(self, plaintext: str, *, timestamp: str, nonce: str) -> dict[str, str]:
        ...
        payload = os.urandom(16) + struct.pack(">I", len(plain_bytes)) + plain_bytes + self.app_id.encode("utf-8")
        cipher = AES.new(self.aes_key, AES.MODE_CBC, self.iv)
